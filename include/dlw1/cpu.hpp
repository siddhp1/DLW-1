...
class Cpu {
 private:
  ...
  uint8_t ReadRegister(const RegisterId id) const;
  void UpdateProcessorStatusWord(const uint8_t result);
  void WriteRegister(const RegisterId id, const uint8_t value);
